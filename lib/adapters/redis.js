(function() {
  ...
  var RedisAdapter = Adapter.extend(function() {
    ...
    .methods({
        ...
        findOne: function findOne(key, callback) {
          this._connector.type(this._keys.roles + ':' + key, function(err, type) {
            if (err)
              return callback(err);
              
            // Retrieve based upon the `type.
            // For now, just IF checks. Later, add `Strategy` pattern.
            if (type === 'hash') {
              
              return this._connector.hgetall(
                this._keys.roles + ':' + key,
                function(err, ok) {
                  if (err)
                    return callback(err);
